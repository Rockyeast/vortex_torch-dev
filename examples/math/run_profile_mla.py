#!/usr/bin/env python
"""Profile vortex sparse-MLA selection quality on a MATH workload (AIME), then
print a report.

Same as examples/ruler/run_profile_mla.py but profiles on math-reasoning prompts
instead of RULER retrieval. Drives a short decode with
...
NOTE: profiling recomputes dense attention in PyTorch per layer per token, so it
is much slower than cuda_mla and runs eager (cuda graph disabled). Use a small
--n. Math prompts are shorter than RULER, so coverage tends to be higher.
"""
import argparse
import json
import os
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data", default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "aime24.jsonl"),
                   help="jsonl with the prompts.")
    p.add_argument("--field", default="question", help="jsonl field holding the prompt text.")
    ...
    p.add_argument("--max-new-tokens", type=int, default=256,
                   help="math answers are longer; give the decode room.")
    p.add_argument("--recall-n", default="16,64,128", help="comma list of N for recall@N.")
    p.add_argument("--out", default="mla_profile_math.json", help="report JSON path.")
...
def render_report(path: str) -> None:
    ...
    print("\n" + "=" * 72)
    print(f"  vortex sparse-MLA selection profile — MATH  ({meta.get('module')})")
    print("=" * 72)
