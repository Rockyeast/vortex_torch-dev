...
OpenAI-API variant of ``auto_agent.py``.  Drives an OpenAI model through
an agentic loop (SDK: ``openai``, Responses API) that:
...
::
    ...
    python examples/misc/auto_agent_oai.py \\
        --submission-name my_agent_v1 \\
        --max-iterations 6 \\
        --min-mean-at-16 0.62 \\
        --baseline-throughput 5500

Only ``OPENAI_API_KEY`` must be set externally.  Everything else has
a safe default.
"""
...
# ---------------------------------------------------------------------------
# Tools exposed to the model (OpenAI Responses API function-calling schema)
# ---------------------------------------------------------------------------
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "timeout_sec": {
                    "type": "integer",
                    "description": "Timeout in seconds; use 1800 unless needed.",
                },
            },
...
def _build_system_prompt() -> str:
    preamble = (
        ...
        "    `python examples/math/run_submission_aime24.py` directly — it "
...
def _build_initial_user_prompt(
    ...
    return (
        ...
        f"   `python examples/math/run_submission_aime24.py` directly — the "
...
def _load_openai() -> Tuple[Any, Any]:
    try:
        import openai
...
def _pretty_response(response: Any) -> str:
    """Short human-readable summary of a Responses API response."""
    out: List[str] = []
    text = getattr(response, "output_text", "") or ""
    if text.strip():
        out.append(f"[text] {text.strip()[:800]}")
    for item in getattr(response, "output", None) or []:
        if getattr(item, "type", None) == "function_call":
            args_preview = (getattr(item, "arguments", "") or "")[:500]
            out.append(
                f"[tool_use name={item.name} call_id={item.call_id} "
                f"args={args_preview}]"
            )
        elif getattr(item, "type", None) == "reasoning":
            out.append(f"[reasoning id={getattr(item, 'id', '')}]")
...
def _call_model(
    ...
    """Call the OpenAI Responses API, retrying transient errors."""
    kwargs: Dict[str, Any] = {
        "model": model,
        "input": input_items,
    ...
    # Reasoning controls are accepted by OpenAI reasoning model families.
    ...
    for attempt in range(4):
        try:
            ...
            # Older openai packages may not expose Responses API-compatible
            # parameters. Surface that as an actionable install issue.
            raise SystemExit(
                "Your installed 'openai' package does not support the "
                "Responses API options used by this script. Upgrade with: "
            ...
            print(f"[api-error attempt {attempt}] {e} — retrying in {wait}s",
                  file=sys.stderr)
            time.sleep(wait)
    raise SystemExit(f"OpenAI API kept failing after 4 retries: {last_err}")
...
def run_agent(
    ...
    openai_module, OpenAI = _load_openai()
    client = OpenAI()
    instructions = _build_system_prompt()
    input_items: Any = _build_initial_user_prompt(
        submission_name,
        min_mean,
        baseline_throughput,
        max_iterations,
    ...
    while turns < max_tool_calls:
        ...
        print(f"\n========== turn {turns} "
              f"(status={status}, "
              f"usage={usage}) ==========")
        print(_pretty_response(response))
        ...
        _log({
            ...
            "usage": usage,
            "content_summary": _pretty_response(response),
        })

        tool_calls = _response_tool_calls(response)

        # --- done? ---
        if not tool_calls and status == "completed":
            print("\n[agent] model ended its turn — stopping.")
            break
        if not tool_calls:
            print(f"\n[agent] no tool calls emitted (status={status!r}) — stopping.")
            break
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="OpenAI-driven sparse-attention submission writer."
    )
    p.add_argument(
        "--submission-name",
...
def main() -> None:
    ...
    print(f"[auto_agent_oai] objective: maximise throughput s.t. "
          f"mean@16 >= {args.min_mean_at_16}")
    print(f"[auto_agent_oai] model              = {args.model}")
    print(f"[auto_agent_oai] submission_name    = {args.submission_name}")
    print(f"[auto_agent_oai] min mean@16 (floor)= {args.min_mean_at_16}")
    if args.baseline_throughput is not None:
        print(f"[auto_agent_oai] baseline_throughput= "
              f"{args.baseline_throughput:.0f} tok/s")
    print(f"[auto_agent_oai] max_iterations     = {args.max_iterations}")
    print(f"[auto_agent_oai] log                -> {log_path}")
